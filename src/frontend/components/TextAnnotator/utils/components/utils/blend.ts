...
export const blender = (tags: Array<Tag>) => {
  ...
  if (overlap.length) {
    // Remove identical positioned tags (best place I could find to fix this in this crazy code)
    overlap = overlap.filter((tag, index) => {
      const overlapIndex = overlap.findIndex((overlapTag) =>
          overlapTag.start === tag.start && overlapTag.end === tag.end
      );
